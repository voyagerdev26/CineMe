...
// inngest function to save user data to a database when user.created event occured by clerk so this handles that and mongodb mai save karta hai
//old syntax
...
const releaseSeatsAndDeleteBooking = inngest.createFunction(
  ...
  async ({ event, step }) => {
    ...
    await step.run("check-payment-status", async () => {
      ...
      if (!booking.isPaid) {
        const show = await Show.findById(booking.show); // so using showId finded show
...
const sendShowReminders = inngest.createFunction(
  {
    ...
    triggers: [{ cron: "0 */8 * * *" }], // every 8 hours this event runs
  },
  async ({ step }) => {
    const now = new Date();
    ...
    const results = await step.run("send-all-reminders", async () => {
      ...
      //       body: "Your movie starts in approximately 8 hours.",
      //     }),
      //   );
      // }
...
// inngest function to send notifications to users when a new show is added by admin
const sendNewShowNotifications = inngest.createFunction(
  ...
  async ({ event }) => {
    ...
    //return response displayed in inngest dashboard
